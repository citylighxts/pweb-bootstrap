...
for (const country of countries) {
    ...
    selectBox.querySelector('ul').appendChild(option);
    options.push(option);
}

function selectOption() {
    const phoneCode = this.querySelector('strong').innerText;
    const flagIcon = this.querySelector('.iconify').outerHTML;
    selectedOption.innerHTML = `<div class="selected-country-option">${flagIcon} <strong>${phoneCode}</strong></div>`;
...
function searchCountry() {
    ...
    options.forEach(option => {
        ...
        option.style.display = isMatched ? 'block' : 'none';
    });
}
